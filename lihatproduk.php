<button type="button" class='btn btn-dis'>PRODUK</button>
<br>
<?php  
$batas =  8;
$halaman = $_GET['pg'];
...
$sqlp = mysqli_query($kon, "select * from tbl_produk order by tglproduk desc limit $posisi, $batas");
  while($rp = mysqli_fetch_array($sqlp)){
	...
	echo "<br><a href='?p=detailproduk&id=$rp[idproduk]'><button type='button' class='btn btn-add'>Detail</button></a>
            <a href='?p=editproduk&id=$rp[idproduk]'><button type='button' class='btn btn-add'>Ubah Produk</button></a>
            <a href='?p=hapusproduk&id=$rp[idproduk]'><button type='button' class='btn btn-add'>Hapus Produk</button></a>";
	echo "</div>";
	echo "</div><br>";
	echo "</div>";
  }
...
$sqlhal = mysqli_query($kon, "select * from tbl_produk");
...
if($halaman > 1){
	$previous=$halaman-1;
	echo "<span class='kotak'><a href=?p=lihatproduk&pg=$previous>&laquo; Prev</a></span> ";
}
else
{ 
...
if($halaman < $jmlhal){
	$next=$halaman+1;
	echo "<span class='kotak'><a href=?p=lihatproduk&pg=$next>Next &raquo;</a></span>";
}
else{ 
